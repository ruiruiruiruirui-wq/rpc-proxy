...
# Binance Smart Chain (BSC) RPC Proxy Test Suite in Shell
# Tests various BSC RPC methods through the OneKey proxy using curl
# Usage: ./test/chain/bsc.sh
...
EOF
    ...
    local http_code=$(curl -s -w "%{http_code}" \
        ...
        --header "User-Agent: OneKey-BSC-RPC-Tester-Shell/1.0" \
        --data "$payload" \
...
# Test: Get chain ID (BSC mainnet = 56)
test_chain_id() {
    ...
    if [[ "$chain_dec" -ne 56 ]]; then
        echo "  Expected BSC chain ID 56, got: $chain_dec"
        return 1
    fi
    
    echo "  ✓ eth_chainId: $chain_hex (decimal: $chain_dec - BSC Mainnet)"
    echo "  ✓ Response time: ${response_time}s"
    return 0
}

# Test: Get network version (BSC mainnet = 56)
test_network_version() {
    ...
    if [[ "$version" != "56" ]]; then
        echo "  Expected BSC network version 56, got: $version"
        return 1
    fi
    
    echo "  ✓ net_version: $version (BSC Mainnet)"
    echo "  ✓ Response time: ${response_time}s"
    return 0
}
...
test_account_balance() {
    ...
    # Convert to BNB (1 BNB = 1e18 Wei)
    local balance_bnb=$(echo "scale=6; $balance_dec / 1000000000000000000" | bc -l)
    
    echo "  ✓ eth_getBalance (latest): $balance_hex ($balance_bnb BNB)"
    echo "  ✓ Response time: ${response_time}s"
    return 0
}
...
# Test: Get contract code (USDT on BSC)
test_contract_code() {
    local usdt_bsc_contract="0x55d398326f99059ff775485246999027b3197955"
    local params='["'$usdt_bsc_contract'", "latest"]'
    ...
    echo "  ✓ eth_getCode (USDT-BSC): Contract code found (${#code} chars)"
    echo "  ✓ Response time: ${response_time}s"
    return 0
}

# Test: Get PancakeSwap factory contract code
test_pancake_factory() {
    local pancake_factory="0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73"
    local params='["'$pancake_factory'", "latest"]'
    local result=$(make_rpc_call "eth_getCode" "$params")
    local response_file=$(echo "$result" | cut -d: -f1)
    local response_time=$(echo "$result" | cut -d: -f2)
    ...
    echo "  ✓ eth_getCode (PancakeSwap Factory): Contract code found (${#code} chars)"
    echo "  ✓ Response time: ${response_time}s"
    return 0
}
...
# Main test execution
main() {
    echo -e "${BLUE}🚀 Starting BSC RPC Proxy Tests (Shell)${NC}"
    
    # Load configuration
    load_config
    
    echo -e "${BLUE}📍 Endpoint: $BSC_URL${NC}"
    ...
    echo -e "\n${YELLOW}📋 BSC RPC Basic Methods${NC}"
    ...
    echo -e "\n${YELLOW}👤 BSC RPC Account Methods${NC}"
