...
local function update_fees()
    local url = "https://mempool.space/signet/api/v1/fees/recommended"
    ...
    local file, err = io.open("/etc/nginx/logs/mempool-signet.conf", "w")
    if not file then
...
local function handle_request()
    local file, err = io.open("/etc/nginx/logs/mempool-signet.conf", "r")
    if not file then
        ngx.log(ngx.ERR, "Failed to open file: ", err)
