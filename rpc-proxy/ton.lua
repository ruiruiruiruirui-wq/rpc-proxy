...
local api_key = os.getenv("TON_API_KEY")
if not api_key or api_key == "" then
    ngx.log(ngx.ERR, "TON_API_KEY not found in environment variables")
end
...
local backends = {
    ton = {
        -- { url = "https://tonhttpapi.mytonwallet.org/api/v2/jsonRPC", host = "tonhttpapi.mytonwallet.org" },
        -- { url = "https://ton.drpc.org/rest/jsonRPC", host = "ton.drpc.org" },
	{ url = "https://toncenter.com/api/v2/jsonRPC", apikey = api_key , host = "toncenter.com" },
        -- { url = "https://rpc.ankr.com/premium-http/ton_api_v2/" .. (api_key) .. "/jsonRPC", host = "rpc.ankr.com" }
    }
}
    ...
    for group, servers in pairs(backends) do
        for _, backend in ipairs(servers) do
            local success, err = pcall(function()
                -- 构建请求 URL，如果有 API key 则添加
                local request_url = backend.url
                if backend.apikey then
                    request_url = request_url .. "?api_key=" .. backend.apikey
                end
                
                local res, request_err = httpc:request_uri(request_url, {
                    method = "POST",
                    body = cjson.encode({
                        jsonrpc = "string",
                        id = "string",
