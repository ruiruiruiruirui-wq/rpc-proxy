...
local backends = {
    apt = {
        { url = "https://rpc.ankr.com/premium-http/aptos/"  .. (ankr_api_key) .. "/", host = "rpc.ankr.com" }
    }
}
    ...
    for group, servers in pairs(backends) do
        for _, backend in ipairs(servers) do
            local success, err = pcall(function()
                local health_check_url = backend.url .. "v1/"
                local res, request_err = httpc:request_uri(health_check_url, {
                    method = "GET",
                    headers = {
