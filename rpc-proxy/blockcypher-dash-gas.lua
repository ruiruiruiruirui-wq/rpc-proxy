...
local function update_fees()
    local url = "https://api.blockcypher.com/v1/dash/main"
    ...
    if not res then
        ngx.log(ngx.ERR, "Failed to request blockcypher API: ", err)
        return
    end
    if res.status ~= 200 then
        ngx.log(ngx.ERR, "Blockcypher API responded with non-200 status: ", res.status)
        return
    end
    local fees = cjson.decode(res.body)
    local file, err = io.open("/etc/nginx/logs/blockcypher-dash-gas.conf", "w")
    if not file then
...
local function handle_request()
    local file, err = io.open("/etc/nginx/logs/blockcypher-dash-gas.conf", "r")
    if not file then
        ngx.log(ngx.ERR, "Failed to open file: ", err)
