...
local function update_fees()
    local url = "https://api.blockcypher.com/v1/ltc/main"
    ...
    local file, err = io.open("/etc/nginx/logs/blockcypher-ltc-gas.conf", "w")
    if not file then
...
local function handle_request()
    local file, err = io.open("/etc/nginx/logs/blockcypher-ltc-gas.conf", "r")
    if not file then
        ngx.log(ngx.ERR, "Failed to open file: ", err)
